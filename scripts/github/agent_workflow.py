...
from hoisa.app.services.issue_quality import (  # noqa: E402
    SUPPORTED_WORK_TYPES,
    classify_issue_type,
    evaluate_issue_quality,
    has_blocking_findings as service_has_blocking_findings,
    issue_quality_report_to_json as service_issue_quality_report_to_json,
    issue_quality_summary_to_json as service_issue_quality_summary_to_json,
)
...
from hoisa.domain.issue_quality import (  # noqa: E402
    IssueQualityComment,
    IssueQualityFinding,
    IssueQualityInput,
    IssueQualityReport,
...
def issue_type(labels: Sequence[str], body: str = "") -> str:
    ...
    return classify_issue_type(labels, body)
...
def issue_quality_report(
    ...
    return evaluate_issue_quality(
        _issue_quality_input(issue),
        _issue_quality_comments(comments),
    )


def _issue_quality_input(issue: IssueItem) -> IssueQualityInput:
...
def _issue_quality_comments(
    comments: Sequence[Mapping[str, Any]],
) -> tuple[IssueQualityComment, ...]:
    quality_comments: list[IssueQualityComment] = []
    for raw in comments:
        ...
        if body:
            quality_comments.append(
                IssueQualityComment(
                    ...
                    source=_issue_quality_comment_source(raw),
                )
            )
    return tuple(quality_comments)


def _issue_quality_comment_source(raw: Mapping[str, Any]) -> str:
...
def issue_quality_report_to_json(report: IssueQualityReport) -> dict[str, Any]:
    ...
    return service_issue_quality_report_to_json(report)
...
def issue_quality_summary_to_json(report: IssueQualityReport) -> dict[str, Any]:
    ...
    return service_issue_quality_summary_to_json(report)
...
def _has_blocking_findings(findings: Sequence[IssueQualityFinding]) -> bool:
    return service_has_blocking_findings(findings)
...
def _eligible_stage_items(items: Sequence[IssueItem], agent: str, mode: Mode) -> list[IssueItem]:
    return _sorted_items(
        ...
        and issue_type(item.labels, item.body) in SUPPORTED_WORK_TYPES
